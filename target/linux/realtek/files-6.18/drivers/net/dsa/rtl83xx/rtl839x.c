...
static void rtl839x_vlan_tables_read(u32 vlan, struct rtldsa_vlan_info *info)
{
	...
	otto_table_read(&buf, RTL8390_TBL_VLAN, vlan);
	u = buf[0];
	v = buf[1];
	w = buf[2];
	...
	otto_table_read(&untag, RTL8390_TBL_UNTAG, vlan);
	u = untag[0];
	v = untag[1];
...
static u64 rtl839x_read_l2_entry_using_hash(u32 hash, u32 pos, struct rtl838x_l2_entry *e)
{
	...
	otto_table_read(&r, RTL8390_TBL_L2_UC, idx);
...
static u64 rtl839x_read_cam(int idx, struct rtl838x_l2_entry *e)
{
	...
	/* Access L2 Table 1 */
	otto_table_read(&r, RTL8390_TBL_L2_CAM_UC, idx);
...
static u64 rtl839x_read_mcast_pmask(int idx)
{
	...
	otto_table_read(&buf, RTL8390_TBL_MC_PMSK, idx);
	portmask = buf[0];
	portmask <<= 32;
	portmask |= buf[1];
...
static int rtldsa_839x_stp_get(struct rtl838x_switch_priv *priv, u16 msti, int port)
{
	...
	otto_table_read(&buf, RTL8390_TBL_MSTI, msti);
...
{
	int r = otto_table_acquire(RTL8390_TBL_MSTI);
	int idx = 3 - ((port + 12) / 16);
	int bit = 2 * ((port + 12) % 16);
	/* port < priv->r->cpu_port (RTL839X_CPU_PORT == 52), so idx is 0..3 */
	u32 buf[4];
...
{
	int q = otto_table_acquire(RTL8390_TBL_IACL);
	u32 r[17];
...
static u32 rtl839x_packet_cntr_read(int counter)
{
	...
	pr_debug("In %s, id %d\n", __func__, counter);
	otto_table_read(&buf, RTL8390_TBL_LOG, counter / 2);
...
{
	int r = otto_table_acquire(RTL8390_TBL_LOG);
	u32 buf[2];
